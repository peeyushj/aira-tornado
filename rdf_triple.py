...
class RDF_Triple():
    ...
    def clear_data(self):
        self.parser = stanford.StanfordParser(model_path="./jars/stanford-parser-3.5.2-models/edu/stanford/nlp/models/lexparser/englishPCFG.ser.gz")
...
#                     new_attr_words.extend(self.extract_word_and_pos(tup[0]))
    ...
    def jsonify_rdf(self):
        return {'sentence':self.sentence,
                'parse_tree':repr(self.parse_tree).decode("utf-8"),
    ...
    def get_subject_word(self, np_array):
        if self.subject.word != '':
            return
        
        for element in np_array:
            self.get_noun(element, True)

    def get_object_word(self, np_array):
        if self.Object.word != '':
            return
        
        for element in np_array:
            self.get_noun(element, False)

    def get_noun(self, element, is_subject):
        try:
            element.label()
        except AttributeError:
            pass
            ...
            if is_subject:
                if element.label()[:2] == 'NN':
                    #print element.leaves()
                    if self.subject.word == '': 
                        self.subject.word = element.leaves()[0]
                        self.subject.pos = element.label()
                        return
                else:
                    for child in element:
                        self.get_noun(child, is_subject)
            else:
                if element.label()[:2] == 'NN':
                    #print element.leaves()
                    if element.leaves()[0] != self.subject.word:
                        if self.Object.word == '': 
                            self.Object.word = element.leaves()[0]
                            self.Object.pos = element.label()
                            return
                else:
                    for child in element:
                        self.get_noun(child, is_subject)
    ...
    def main(self):
        self.clear_data()
        self.parse_tree = self.parser.raw_parse(self.sentence).next()
        #self.parse_noun_tree = self.add_nouns(self.parse_tree)
        ...
        if self.subject.word == '' and self.first_NP != '':
            self.get_subject_word(self.NP_array)
            if self.subject.word == '':
        ...
        self.find_object()
        if self.Object.word == '' or self.Object.word==self.subject.word:
            self.Object.word = ''
            self.get_object_word(self.NP_array)
